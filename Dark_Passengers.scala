...
class DarkPassengersQueen extends Queen {
  def firstPlay = {
    if (countAnts < 2) layEgg(new Harry(400, 20, 0, 200))
    else if(countAnts<3) layEgg(new Lila(36, 3, false, 0))
    if (countAnts < 5){
      if (countAnts%3 == 1) layEgg(new Rita(400, 20, -10, 240))
      else if (countAnts%3 == 0) layEgg(new Rita(400, 20, 10, 240))
    }
    else if(countAnts < 7){
      if(countAnts%2 == 1) layEgg(new Batista(22, 3, 0))
      else if(countAnts%2 == 0) layEgg(new Lila(24, 10, false, 0))
    }
    else if(countAnts < 9){
      layEgg(new Harry(600, 10, 0, 250))
    }
}
  ...
  def takeTurn = {
    ...
    if(food > 2500){
      if(countAnts < 8){
        layEgg(new Lila(2, 3, false, 0))
      }
      else if(countAnts < 10){
        layEgg(new Lila(37, 5, false, 0))
      }
      else if(countAnts < 11){
        if(lookForMessage(East) != 10)
          layEgg(new Batista(1, 2, 2))
        else layEgg(new Lila(37, 4, true, 1))
      }
      else{
        layEgg(new Doakes(2))
      }
    }
    else if(food < 1000 && countAnts < 4){
        layEgg(new Lila(3, 3, true, 1))
    }
    else{
      val selector = random(3)
      if(selector == 0) layEgg(new Rita(400, 10, 10, 0))
      else if(selector == 1) layEgg(new Rita(400, 10, -10, 0))
      else layEgg(new Rita(400, 10, 0, 0))
    }
  }
}
...
abstract class Family extends Ant{
  ...
  val blacklist = scala.collection.mutable.ArrayBuffer.empty[Direction]
  def moveTarget(x: Int, y: Int): Unit = {
     if(food > 0){
       val dif_x = Math.abs(pos.x)-Math.abs(x)
       val dif_y = Math.abs(pos.y)-Math.abs(y)
       if(dif_x*dif_x+dif_y*dif_y <= 2) drop
     }
     if(pos.x > x+1 && !blacklist.contains(West)){
        if(canMove(West)) move(West)
        else{
          blacklist.append(West)
          moveTarget(x, y)
        }
      }
      else if(pos.y > y && !blacklist.contains(South)){
        if(canMove(South)) move(South)
        else{
          blacklist.append(South)
          moveTarget(x, y)
        }
      }
      else if(pos.y < y && !blacklist.contains(North)){
        if(canMove(North)) move(North)
        else{
          blacklist.append(North)
          moveTarget(x, y)
        }
      }
      else if(pos.x < x && !blacklist.contains(East)){
        if(canMove(East)) move(East)
        else{
          blacklist.append(East)
          moveTarget(x, y)
        }
      }
      blacklist.clear()
    ...
    def squiggle(center: Int, prev: Direction): Direction = {
      ...
      val choose = random(4)
      if( Math.abs(pos.y)-Math.abs(center) > 7){
        if(pos.y > center){
          if(canMove(South)) return South
          else if(canMove(East)) return East
          else if(canMove(North)) return North
          else move(West)
        }
        else{
          if(canMove(North)) return North
          else if(canMove(East)) return East
...
class Harry(cap: Int, x: Int, y: Int, expTime: Int) extends Family { //Get food from center
    icon = 'M'
    capacity = cap //make function to generate optimal capacity given
    strength = 1
    health = 8
